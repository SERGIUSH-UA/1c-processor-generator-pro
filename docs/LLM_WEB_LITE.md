...
### ObjectModule Region (v2.66.0+)

Write ObjectModule code directly in `handlers.bsl`:
```bsl
#Область МодульОбъекта

Функция СведенияОВнешнейОбработке() Экспорт
    // Goes to ObjectModule.bsl (auto-wrapped with #Если Сервер...)
КонецФункции
...
| Reserved keyword as handler | ❌ `Выполнить` → ✅ `ВыполнитьОбработку` |
| Ukrainian in identifier | ❌ `ПошуковийЗапит` (і) → ✅ `ПоисковыйЗапрос` (и) |
| Random numbers | ❌ `СлучайноеЧисло()` → ✅ `Новый ГенераторСлучайныхЧисел()` |
| ObjectModule attr access | ❌ `Объект.Атр` → ✅ `Атр` (без Объект.) |
