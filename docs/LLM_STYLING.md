...
## 10. Complete Styled Form Example

```yaml
processor:
  name: СтилизованнаяФорма
  synonym_ru: Styled Form Example
...
forms:
  - name: Форма
    ...
    # Conditional row styling (v2.68.0+)
    conditional_appearances:
      - name: ОшибкиКрасные
        selection: [СтатусКолонка, НазваниеКолонка, СуммаКолонка]
        filter:
          field: Результаты.Статус
          comparison: Equal
          value: "Ошибка"
    ...
    value_tables:
      - name: Результаты
        columns:
          - {name: Название, type: string, length: 100}
          - {name: Сумма, type: number, digits: 15, fraction_digits: 2}
          - {name: Статус, type: string, length: 20}

    elements:
      # Styled header (font with size)
      - type: LabelDecoration
        name: Заголовок
        title_ru: "Styled Form Demo"
        font:
          size: 18
          bold: true
        horizontal_align: Center

      # Info message with HTML
      - type: LabelDecoration
        name: Инфо
        formatted: true
        title: "<font color='#1976D2'>Fill the form and click</font> <b>Execute</b>"

      # Input group with styling
      - type: UsualGroup
        name: ВводГруппа
        title_ru: "Input"
        show_title: true
      ...
      # Results table with height control
      - type: Table
        name: РезультатыТаблица
        tabular_section: Результаты
        is_value_table: true
        height_in_table_rows: 10      # 10 visible rows
        header_height: 2
        horizontal_stretch: true
        elements:
          - type: InputField
            name: НазваниеКолонка
            attribute: Название
            width: 30
            title_font:
              size: 11
    ...
    commands:
      ...
      - name: Очистить
        ...
        picture: StdPicture.InputFieldClear
```
...
- [TABLE_STYLING.md](reference/TABLE_STYLING.md) - Detailed table styling reference
- [CONDITIONAL_APPEARANCE.md](reference/CONDITIONAL_APPEARANCE.md) - Full conditional appearance guide
