...
### 2.5. Tree (Hierarchical Data) - v2.64.0+
```yaml
forms:
  - name: Форма
    default: true
...
```bsl
// Add root node
Root = DataTree.GetItems().Add();
Root.Name = "Root";

// Add child nodes
Child = Root.GetItems().Add();
Child.Name = "Child 1";

// Navigate tree
Parent = Node.GetParent();  // Get parent
Children = Node.GetItems(); // Get children
```
...
**New in 2.12.0:** CheckConfig Semantic Validation - deep semantic checks (empty handlers, unreferenced procedures, incorrect references) with YAML configuration
**New in 2.11.1:** BSL Validation through /CheckModules - automatic syntax checking with dual-format log parsing, error classification, and XDTO fixes (cfg: prefix, XML formatting)
